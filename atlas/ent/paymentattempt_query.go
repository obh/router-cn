...
func (paq *PaymentAttemptQuery) QueryPaymentIntent() *PaymentIntentQuery {
	query := (&PaymentIntentClient{config: paq.config}).Query()
...
func (paq *PaymentAttemptQuery) FirstID(ctx context.Context) (id int, err error) {
	var ids []int
...
func (paq *PaymentAttemptQuery) FirstIDX(ctx context.Context) int {
...
func (paq *PaymentAttemptQuery) OnlyID(ctx context.Context) (id int, err error) {
	var ids []int
...
func (paq *PaymentAttemptQuery) OnlyIDX(ctx context.Context) int {
...
func (paq *PaymentAttemptQuery) IDs(ctx context.Context) (ids []int, err error) {
...
func (paq *PaymentAttemptQuery) IDsX(ctx context.Context) []int {
	ids, err := paq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (paq *PaymentAttemptQuery) Clone() *PaymentAttemptQuery {
	...
	return &PaymentAttemptQuery{
		...
		withPaymentIntent: paq.withPaymentIntent.Clone(),
...
func (paq *PaymentAttemptQuery) WithPaymentIntent(opts ...func(*PaymentIntentQuery)) *PaymentAttemptQuery {
	query := (&PaymentIntentClient{config: paq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	paq.withPaymentIntent = query
	return paq
}
...
//		GroupBy(paymentattempt.FieldPaymentIntentID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.PaymentAttempt.Query().
//		Select(paymentattempt.FieldPaymentIntentID).
//		Scan(ctx, &v)
...
func (paq *PaymentAttemptQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*PaymentAttempt, error) {
	...
	if query := paq.withPaymentIntent; query != nil {
		if err := paq.loadPaymentIntent(ctx, query, nodes, nil,
			func(n *PaymentAttempt, e *PaymentIntent) { n.Edges.PaymentIntent = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (paq *PaymentAttemptQuery) loadPaymentIntent(ctx context.Context, query *PaymentIntentQuery, nodes []*PaymentAttempt, init func(*PaymentAttempt), assign func(*PaymentAttempt, *PaymentIntent)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*PaymentAttempt)
	for i := range nodes {
		fk := nodes[i].PaymentIntentID
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "payment_intent_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (paq *PaymentAttemptQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(paymentattempt.Table, paymentattempt.Columns, sqlgraph.NewFieldSpec(paymentattempt.FieldID, field.TypeInt))
	_spec.From = paq.sql
	...
	if fields := paq.ctx.Fields; len(fields) > 0 {
		...
		if paq.withPaymentIntent != nil {
			_spec.Node.AddColumnOnce(paymentattempt.FieldPaymentIntentID)
		}
	}
